...
export function PrivateCollections({ onOpenModal }: { onOpenModal: (property: any) => void }) {
  return (
    <section id="collections" className="bg-charcoal py-32 border-b border-white/5 overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="sm:hidden flex overflow-x-auto snap-x-mandatory gap-4 -mx-6 px-6 no-scrollbar pb-4">
          {collections.map((collection, idx) => (
            <div 
              key={idx}
              className="snap-start shrink-0 w-[72vw] h-[340px] relative rounded-[4px] overflow-hidden border border-white/10 bg-charcoal-light cursor-pointer active:scale-[0.98] transition-transform"
              onClick={() => onOpenModal({
                id: `collection-${idx}`,
            ...
            >
              ...
              <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md border border-white/20 text-gold text-[10px] font-mono px-3 py-1 rounded-full uppercase tracking-wider font-semibold">
                ✓ {collection.optionsCount} Verified
              </div>

              {/* Title and Price Bottom Left */}
              <div className="absolute bottom-4 left-4 right-4 text-left">
                <h4 className="text-xl font-serif text-ivory mb-1 leading-snug drop-shadow-md">
                  {collection.title}
                </h4>
          ...
          {collections.map((collection, idx) => (
            ...
            >
              <Tilt tiltMaxAngleX={3.5} tiltMaxAngleY={3.5} scale={1.02} transitionSpeed={2500}>
                <div className="relative aspect-[4/5] sm:aspect-[3/4] overflow-hidden bg-charcoal-light border border-white/5 mb-3 sm:mb-6">
                  <div className="absolute inset-0 bg-charcoal/30 group-hover:bg-charcoal/10 transition-colors duration-1000 z-10" />
                  <div className="absolute inset-0 z-10 bg-gradient-to-t from-charcoal/90 via-charcoal/30 to-transparent opacity-90 group-hover:opacity-80 transition-opacity duration-1000" />
                  
                  <img 
                    src={collection.image} 
                    alt={collection.title} 
                    className="w-full h-full object-cover transform scale-[1.03] group-hover:scale-110 transition-transform duration-[3s] ease-lux grayscale-[20%] group-hover:grayscale-0"
                  />
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-5 sm:p-8 z-20 flex flex-col justify-end">
                    ...
                    <h4 className="text-xl sm:text-3xl lg:text-4xl font-serif text-ivory mb-1 leading-snug drop-shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform duration-700 delay-75 ease-lux">
                      {collection.title}
                    </h4>
                  </div>
                </div>
              ...
              <div className="pl-4 sm:pl-5 border-l border-white/10 flex-1 flex flex-col justify-between">
                <p className="text-bone/70 text-[11px] sm:text-sm font-light mb-4 sm:mb-6 leading-[1.8] line-clamp-2 md:line-clamp-3">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-2">
                  <div className="text-ivory font-medium flex flex-col gap-1 sm:block">
                    ...
                    <span className="text-sm sm:text-base font-serif">{collection.startingPrice}</span>
                  </div>
